...
interface SpecialtyDoctorListProps {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                >
                    <div className="flex-1 min-w-0 mb-5">
                        ...
                        <div className="flex flex-wrap items-center gap-y-1 gap-x-3 text-sm font-medium text-[#86868b]">
                           {doc.cities.slice(0, 2).map((c, i) => (
                               <span key={i} className="flex items-center gap-1.5">
                                  <MapPin className="w-4 h-4 text-[#86868b]/70" /> {c}
                               </span>
                           ))}
                           {doc.cities.length > 2 && <span>+{doc.cities.length - 2} más</span>}
                        </div>
                    </div>
            ...
            {doctors.length === 0 && (
                <div className="col-span-full py-20 text-center text-[#86868b]">
                    ...
                    <p className="text-lg">No encontramos doctores verificados para {specialty} en este momento.</p>
