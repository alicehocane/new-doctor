...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  // We keep standard select for metadata as the RPC might not return SEO fields
...
export default async function DoctorProfile({ params }: { params: { slug: string } }) {
  ...
  // 1. Fetch Main Doctor Data using RPC
  // The RPC 'get_complete_doctor_profile' returns a JSON object with aggregated relations
  const { data: rpcData, error } = await supabase
    .rpc('get_complete_doctor_profile', { doctor_slug: params.slug });
  ...
  // 3. Transform RPC Data to Doctor Interface
  // Note: The RPC might not return all fields (like ID or SEO), so we handle defaults.
  const doctor: Doctor = {
    id: (rpcData as any).id || '', // RPC currently missing ID, will default to empty
    external_id: null,
    slug: params.slug,
    full_name: rpcData.full_name,
    specialties: rpcData.specialties || [],
    cities: rpcData.cities || [],
    license_numbers: rpcData.license_numbers || [],
    contact_info: rpcData.contact_info || { locations: [], phones: [] },
    medical_profile: {
      diseases_treated: rpcData.medical_profile?.diseases_treated || [],
      sub_specialties: rpcData.medical_profile?.focused_on || [] // Map 'focused_on' from RPC to 'sub_specialties'
    },
    // Fields potentially missing in RPC, defaulting to ensure UI doesn't crash
    seo_metadata: (rpcData as any).seo_metadata || { meta_title: '', meta_description: '', keywords: '' },
    schema_data: {},
    created_at: new Date().toISOString(), 
    updated_at: (rpcData as any).updated_at || new Date().toISOString()
  };
  ...
  const relatedDoctorsPromise = (async () => {
    if (doctor.cities.length > 0 && doctor.specialties.length > 0) {
      ...
      const { data: related } = await supabase
        ...
        // .neq('id', doctor.id) // Cannot filter by ID if RPC doesn't return it
        .limit(15);
      ...
      if (related) {
        ...
        // Filter out current doctor by slug (since we might lack ID)
        const filteredRelated = formattedRelated.filter((d: any) => d.slug !== params.slug);
